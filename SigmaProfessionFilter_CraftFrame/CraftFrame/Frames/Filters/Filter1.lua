...
SPF.Filter1 = CreateFrame("CheckButton", "CraftFilter1Button", CraftFrame, "UICheckButtonTemplate");
...
function SPF.Filter1.OnLoad()
	...
	SPF["CFOnShow"]["SPF.Filter1.OnShow"] = SPF.Filter1.OnShow;
...
function SPF.Filter1:OnShow()
	...
	if GetCraftName() then
		SPF.Filter1:SetChecked(SPF.Filter1.Status[GetCraftName()]);
	end
end
...
function SPF.Filter1.OnClick()
	if (arg1 == "RightButton") then
		...
		if GetCraftName() then
			SPF.Filter1.Status[GetCraftName()] = SPF.Filter1:GetChecked();
		end
	end
...
function SPF.Filter1:Filter(skillIndex)
	if SPF:Custom("Filter1").Filter then
		...
		local _, _, skillType = SPF.baseGetCraftInfo(skillIndex);
...
function SPF.Filter1:FilterSpell(spellID)
	...
	local levels = SPF.GetCraftInfo(spellID, "levels");
	local _, level = GetCraftDisplaySkillLine();
